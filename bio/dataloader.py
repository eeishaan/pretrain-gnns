...
class DataLoaderFinetune(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderFinetune, self).__init__(
            ...
            batch_size,
            shuffle,
            collate_fn=lambda data_list: BatchFinetune.from_data_list(
                data_list),
            **kwargs)
...
class DataLoaderMasking(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderMasking, self).__init__(
            ...
            batch_size,
            shuffle,
            collate_fn=lambda data_list: BatchMasking.from_data_list(
                data_list),
            **kwargs)
...
class DataLoaderMaskingSubstructContext(torch.utils.data.DataLoader):
    r"""Data loader which merges data objects from a
    :class:`torch_geometric.data.dataset` to a mini-batch.
    Args:
        dataset (Dataset): The dataset from which to load the data.
    ...
    def custom_collate_fn(data_list):
        return BatchMasking.from_data_list(data_list), BatchSubstructContext.from_data_list(data_list)

    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderMaskingSubstructContext, self).__init__(
            dataset,
            batch_size,
            shuffle,
            collate_fn=DataLoaderMaskingSubstructContext.custom_collate_fn,
            **kwargs)
