...
def main():
    ...
    parser.add_argument('--infograph_model', type=bool, default=False,
                        help='If true, will use the info-graph model instead of the default_one')
    ...
    model = GNN_graphpred(args.num_layer, args.emb_dim, num_tasks, JK=args.JK, drop_ratio=args.dropout_ratio,
                          graph_pooling=args.graph_pooling, gnn_type=args.gnn_type,info_graph_mode=args.infograph_model)
    if args.infograph_model:
        model.gnn = Encoder(args.emb_dim, args.num_layer)
    ...
    if not args.filename == "":
        with open("result/finetune_seed" + str(args.runseed) + "/" + args.filename, 'wb') as f:
            if args.split == "random":
                pickle.dump(
                    {"train": np.array(train_acc_list), "val": np.array(val_acc_list), "test": np.array(test_acc_list)},
                    f)
            else:
                pickle.dump({"train": np.array(train_acc_list), "val": np.array(val_acc_list),
                             "test_easy": np.array(test_acc_easy_list), "test_hard": np.array(test_acc_hard_list)}, f)
